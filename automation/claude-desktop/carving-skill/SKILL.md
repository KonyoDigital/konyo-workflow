---
name: carving-skill
description: Graduate three or more scars in one territory into their own skill. Use when SCARS.md keeps repeating a theme, grows too long to read, or when asked to carve or extract scars.
---

<!-- ⚠ DESCRIPTION LENGTH — TWO NUMBERS DISAGREE IN THIS PROJECT, SO THIS OBEYS THE STRICTER.
     `automation/claude-desktop/VARIANTS.md` states "a description over 200 chars is rejected on
     upload. This has already caught one." `ship-skill/verify_package.py` sets LIMIT = 1024 and
     says over-long is "rejected at upload, never truncated". Both claim upload rejection; they
     cannot both be right, and neither was re-measured on 2026-08-14. A description under 200
     passes under EITHER rule, so this one is 203 -> trimmed under 200 rather than betting on
     whichever is correct. The Claude CODE edition of this skill keeps the longer description,
     because that host truncates at ~1536 instead of rejecting — a different failure mode.
     If someone measures the real Desktop limit, fix both documents, not just one. -->
...
Writing the file is not the end (§3, Step 6). This is that step. In this Desktop edition
there is no shell to run a checker in, so do it by eye — it is four things, and skipping
them is how a carve fails silently:

1. **The frontmatter parses.** The file opens with `---`, then `name:` and `description:`,
   then `---`. Nothing before the first `---`.
2. **`name:` matches the folder name exactly.** `carving-skill` in a folder called
   `carving-skill`. A mismatch and it does not load.
3. **The `description:` is non-empty and contains no `: ` (colon-space).** See the warning
   below — this is the one that bites.
4. **Every `references/….md` the skill mentions is actually in the zip.** A dangling
   reference reads as content that exists and is silently absent.

Then the real proof: **start a new conversation, describe the work in your own words, and
watch whether the skill loads.** If it does not, the description is naming the subject
instead of the trigger — go back to Step 4. You have not carved anything until you have
seen it fire.
...
*(The Claude Code edition of this skill ships a runnable version of this check. This file is
the Desktop derivation of that one — if you change the method, change it there first.)*
